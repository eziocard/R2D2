...
def resolver_respuesta(answers, predicted_category, phrase_user):
    respuesta = answers.get(predicted_category)

    if isinstance(respuesta, dict):
        frase_norm = phrase_user.lower().strip()
        respuesta  = respuesta.get(frase_norm) or random.choice(respuesta["default"])
...
def infer(model, tokenizer, categories, answers):
    ...
    while True:
        ...
        if phrase_user.lower() in ["salir","chao", "exit", "quit"]:
            print("[R2D2]: Hasta luego, Que la fuerza de acompañe")
            break
        ...
        print(
            f" [R2D2 DETECTA]: Intención -> '{predicted_category}'"
            f" | Confianza -> {level_confidence:.1f}%"
        )

        if level_confidence < 45:
            print("[R2D2]: No estoy seguro de haber entendido. ¿Puedes reformularlo?")
            continue

        respuesta = resolver_respuesta(answers, predicted_category, phrase_user)
        print(f"[R2D2]: {respuesta}")
